...
class TC{
  ...
  public static int DIJS(Grafo2 g, int or, int dest){
    ...
    while(!q.isEmpty()){
      ...
      for (Arco y : g.adjs_no(v)) {
        ...
        if (y.valor1_arco()==0) continue;
        if (dist[w]>dist[v]+y.valor0_arco()) {
          dist[w]=dist[v]+y.valor0_arco();
          q.decreaseKey(w, dist[w]);
        }
      }
    }
    if (dist[dest]<1000) return dist[dest];
    else return 0;
  }
  ...
  public static void main(String[] args) {
    ...
    for (int i=0; i<nt; i++) {
      ...
      for (int j=1; j<nlig; j++) {
        ...
        if (g.find_arc(p1, p2)==null) {
          g.insert_new_arc(p1, p2, d, 1);
          g.insert_new_arc(p2, p1, d, 1);
        }
        p1=p2;
      }
    }
    ...
    for (int i=0; i<mod; i++) {
      ...
      if (amax!=-1){
        if (amax<med) {
          g.find_arc(p2,p1).novo_valor1(0);
          g.find_arc(p1,p2).novo_valor1(0);
        }
       }
      if (lmax!=-1) {
        if (lmax<med) {
