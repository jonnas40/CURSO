...
class POI {
  ...
  public static int[] DIJS(Grafo2 g, int or, int k){
    int[] dist = new int[g.num_vertices()+1];
    val = new boolean[g.num_vertices()+2];
    for (int i=0; i<dist.length; i++) dist[i]=999999;
    dist[or]=0;
    val[or]=true;
    ...
    while (!q.isEmpty()) {
      ...
      for (Arco y : g.adjs_no(v)) {
        ...
        if (dist[w]>dist[v]+y.valor1_arco() && k<=y.valor0_arco()) {
        ...
        /*if (k<=y.valor0_arco() && val[v]) {
          //System.out.println("no: " + w + " " +k + " " + y.valor0_arco());
          val[w]=true;
        }*/
      }
    }
  ...
  public static void main(String[] args) {
    ...
    for (int i=0; i<nlig; i++) g.insert_new_arc(sc.nextInt(), sc.nextInt(), sc.nextInt(), sc.nextInt());
    int[] sol = new int[nnos];
    sol = DIJS(g, or, k);
    boolean flag=false;
    for (int i=1; i<sol.length; i++) {
      ...
      if (val[i] && i!=or) {
        if (sol[i]<=m) {
          System.out.println("Destino "+i+": " + sol[i]);
          flag=true;
        }
      }
    }
