...
class OM {
  ...
  public static void Prim(Grafo g, int s){
    ...
    dist[s]=0;
    Heapmax q = new Heapmax(dist, g.num_vertices()+1);
    Queue<Arco> fila = new Queue<Arco>();
    while(!q.isEmpty()){
      int v=q.extractMax();
      fila.add(find_arc(pai[v], v));
      ok[v]=true;
      for (Arco y : g.adjs_no(v)) {
        int w = y.extremo_final();
        if(!ok[w] && g.valor_arco(v, w)<dist[w]){
          dist[w]=g.valor_arco(v, w);
          pai[w]=v;
  ...
  public static void main(String[] args) {
    ...
    int[] vec = new int[nnos+1];
    for (int i=1; i<=nlig; i++) {
      g.insert_new_arc(nextInt(), nextInt(), nextInt());
    }
